...
- **PII / data handling (revised in Stage 3):** OnTrack will use Gmail login with 2-step verification for accounts, which means collecting at least an email address (and likely name/profile picture via Google's OAuth) — this supersedes the earlier "no personal info" note, which was actually about a narrower concern: not having the (deferred) Canvas integration pull other people's names/emails without permission. Separately, if the Canvas auto-carryover feature is ever built, it requires the student to authorize access to their real Canvas account — an access token tied to a real school account, with assignment data pulled from a school system. Both points need real handling (secure storage, no unnecessary retention) rather than being treated as "no data handling needed."
...
- Job 3 (Canvas auto-carryover) was considered but not built out — two jobs were judged sufficient coverage for Stage 2. Still available as a candidate job if Stage 3 screen design needs more grounding for that feature.
